...
#### parse(rpcString, validatorConfig)

Parses the rpc string into an object.

`var rpcObj = rpc.parse(rpcString, validatorConfig);`
...
#### extendPaths(rpcObj, validatorConfig, mountPath)

Extends path-args by the `mountPath` string.

`rpc.extendPaths(rpcObj, validatorConfig, mountPath);`
